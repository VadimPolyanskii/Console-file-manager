...
from functions_for_tests import *      # Модуль с функциями для тестов
...
def test_filter_even_numbers_with_non_integers():
    ...
    assert filter_even_numbers([1.0, -2.5, 4.0]) == [4.0]
...
def test_map_for_number():
    ...
    assert map_for_number([1, 2, 3, 4, 5]) == [1, 4, 9, 16, 25]

def test_map_for_strings():
    # Тест преобразования строк в целые числа
    assert map_for_strings(["1", "2", "3"]) == [1, 2, 3]


def test_map_for_strings_upper():
    # Тест возведения строк к верхнему регистру
    assert map_for_strings_upper(["возведение", "строк", "к верхнему", "регистру"]) == ["ВОЗВЕДЕНИЕ", \
...
def test_sorted_list_number():
    ...
    assert sorted_list_number([3, 2, 1]) == [1, 2, 3]


def test_sorted_list_number_revers():
    # Тест сортировки в обратном порядке
    assert sorted_list_number_revers([4, 1, 9, 2, 7]) == [9, 7, 4, 2, 1]


def test_sorted_string():
    """Тест сортировки строки"""
    assert sorted_string("mamba") == ["aabmm"]


def test_sorted_by_key():
    """Тест сортировки по длине слов"""
    assert sorted_by_key(["banana", "apple", "orange", "pie"]) == ["pie", "apple", "banana", "orange"]


def test_sort_students():
    # Тест сортировки по возрасту
    assert sort_students([{'name': 'Alex', 'age': 22},
                                              {'name': 'Alice', 'age': 19},
                                              {'name': 'Mat','age': 20},
...
def test_sort_with_multiple_keys():
    ...
    assert sort_with_multiple_keys([('apple', 3, 2),
                                                        ('orange', 1, 4),
                                                        ('pear', 2, 4),
...
def test_calculating_square_root_of_a_fractional_number():
    ...
    assert calculating_square_root_of_a_fractional_number(-1) == (None, 'math domain error')


def test_calculating_the_square_root_of_a_fractional_number():
    # Тест вычисления квадратного корня из дробного числа
    assert calculating_the_square_root_of_a_fractional_number(2.25) == 1.5


# pow
def test_raising_to_a_power():
    # Тест возведения в степень
    assert raising_to_a_power(2, 3) == 8.0


# hypot
def test_calculating_hypotenuse():
    # Тест вычисления гмпотенузы
    assert calculating_hypotenuse(3, 4) == 5.0
...
def test_working_with_coordinates():
    ...
    dx = 4 - 1
    dy = 6 - 2
    assert working_with_coordinates(dx, dy) == 5.0


def test_vector_norm():
    # Тест векторной нормы; вклидова норма вектора (3, 4, 12)
    vector = (3, 4, 12)
    assert vector_norm(vector) == 13.0